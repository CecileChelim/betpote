...
export default function ShareButtons({ betId, title }) {
  ...
  const waText = encodeURIComponent(`🎲 *BetPote* — "${title}"\nVote et rejoins le pari !\n👉 ${url}`)
  function copyLink() {
    navigator.clipboard.writeText(url).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000) })
  }
  return (
    <div className="flex flex-col gap-3">
      <a href={`https://wa.me/?text=${waText}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2.5 bg-[#25D366] text-white font-bold text-base rounded-2xl py-4 hover:bg-[#1EB658] transition-colors">
